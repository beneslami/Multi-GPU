...
public:
    ...
    bool send(Request req)
    {
        ...
        if (ctrls[req.addr_vec[0]]->enqueue(req)) {
            ...
            //printf("KAIN Stack %d, Channel %d, Rank %d, BankGroup %d, Bank %d, Row %d, Column %d\n",kain_DRAM_id, req.addr_vec[int(HBM::Level::Channel)], req.addr_vec[int(HBM::Level::Rank)],req.addr_vec[int(HBM::Level::BankGroup)] ,req.addr_vec[int(HBM::Level::Bank)], req.addr_vec[int(HBM::Level::Row)], req.addr_vec[int(HBM::Level::Column)]);
            //fflush(stdout);
            return true;
        }
        return false;
