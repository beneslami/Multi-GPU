#ifndef __SHD_WARP_T_H__
#define __SHD_WARP_T_H__
...
public:
    ...
    void ibuffer_flush()
    {
        for(unsigned i=0;i<IBUFFER_SIZE;i++) {
            if( m_ibuffer[i].m_valid ) {
                ...
                //dec_inst_in_pipeline(m_ibuffer[i].m_inst);
            }
    ...
    void clear_imiss_pending() { assert(imiss_pending()); m_imiss_pending=false; }
    ...
    /*
    bool inst_in_pipeline() const { return !m_warp_insts_in_pipeline.empty(); }
    void inc_inst_in_pipeline(const warp_inst_t* inst) { m_warp_insts_in_pipeline.push_back(inst); }
    void dec_inst_in_pipeline(const warp_inst_t* inst)
    {
        std::list<const warp_inst_t*>::iterator it = std::find(m_warp_insts_in_pipeline.begin(), m_warp_insts_in_pipeline.end(), inst);
        assert( it != m_warp_insts_in_pipeline.end() );
        m_warp_insts_in_pipeline.erase(it);
    }
    void flush_insts()
    {
      for (std::list<const warp_inst_t*>::iterator it = m_warp_insts_in_pipeline.begin(), it_end = m_warp_insts_in_pipeline.end();
           it != it_end; ++it) {
        (*it)->clear();
      }
      m_warp_insts_in_pipeline.clear();
    }
    */
    ...
    void dec_inst_in_pipeline(const unsigned num_insts)
    {
        assert( m_inst_in_pipeline >= num_insts );
        m_inst_in_pipeline -= num_insts;
    }

    bool is_pending_for_migration() const { return !stores_done() || inst_in_pipeline() || imiss_pending(); }
    ...
    void load_warp_context(warp_context_t* loading_ctx, unsigned cta_id, unsigned wid);
...
private:
    ...
    //std::list<const warp_inst_t*> m_warp_insts_in_pipeline;
};

#endif // __SHD_WARP_T_H__
